...
const TrackerScoreSync = () => {
  ...
  const identifyUser = useCallback((ev: CustomEvent<{ userId?: string }>) => {
  ...
  // ToDo: for SSG + ESP
  useEffect(() => {
    const userId = cookies[STORE_USER_COOKIE_NAME] || '';
    const anonymousId = cookies[STORE_ANONYMOUS_ID_COOKIE_NAME] || '';
    if (userId || anonymousId) {
      fetch(`/api/user/traits?user_id=${userId}&anonymous_id=${anonymousId}`)
  ...
  useEffect(() => {
    ...
    if (!interest) return;
    global?.analytics?.identify(cookies[STORE_USER_COOKIE_NAME], { interest });
  }, [scores, cookies]);
