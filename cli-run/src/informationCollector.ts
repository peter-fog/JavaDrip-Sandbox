...
import { select, text, intro, confirm, note } from './promts';
import { addExamplesCanvasCache, remove } from './utils';
import path from 'path';
import { Integrations } from './mappers';
import { fixEslint, installPackages } from './commands/run';
import { configureIntegration } from './commands/setupUniform/utils';
import fs from 'fs';
...
export const additionalModulesForComponentStarterKit =
  ...
  async ({ progressSpinner, projectPath, uniformAccessData }: CLI.AdditionalModulesExecutorProps) => {
    ...
    const isRunAddingModules = await confirm({
      message: `Do you want to push additional Examples (Coveo Search)?`,
    });
    ...
    if (
      uniformAccessData?.teamId &&
      uniformAccessData?.projectId &&
      uniformAccessData?.uniformApiHost &&
      uniformAccessData?.headers
    ) {
      for (const integration of [Integrations.Coveo]) {
        ...
        await configureIntegration({
          ...
          teamId: uniformAccessData.teamId,
          projectId: uniformAccessData.projectId,
          customManifest: integration.customManifest,
          apiHost: uniformAccessData.uniformApiHost,
          headers: uniformAccessData.headers,
        });
        progressSpinner.stop(`Finished installing ${integration.name} integration`);
      }
    } else if (integrationList.length) {
      note(
        color.red(
          `Looks like we don't have enough data to set the integrations automatically. Please check and set manually if necessary: ${integrationList
            .map(i => i.name)
            .join(', ')}.`
        )
      );
    ...
    const coveoOrganizationId = process.env.CLI_COVEO_ORGANIZATION_ID;
    const coveoApiKey = process.env.CLI_COVEO_API_KEY;
    const pathToEnvFile = path.resolve(projectPath, '.env');
    const envVars = await fs.promises.readFile(pathToEnvFile, 'utf-8');
    await fs.promises.writeFile(
      pathToEnvFile,
      `${envVars}\nNEXT_PUBLIC_COVEO_ORGANIZATION_ID=${coveoOrganizationId}\nNEXT_PUBLIC_COVEO_API_KEY=${coveoApiKey}`
    );
